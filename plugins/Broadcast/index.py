...
async def index_file(bot, query):
    ...
    if raju == 'reject':
        ...
        await bot.send_message(int(from_user),
                               f'Your Submission for indexing {chat} has been decliened by {temp.B_NAME} moderators.',
    ...
    if int(from_user) not in ADMINS:
        await bot.send_message(int(from_user),
                               f'Your Submission for indexing {chat} has been accepted by {temp.B_NAME} moderators and will be added soon.',
                               reply_to_message_id=int(lst_msg_id))
    await msg.edit(
        "Starting Indexing",
        reply_markup=InlineKeyboardMarkup(
...
async def send_for_index(bot, message):
    ...
    try:
        ...
        return await message.reply(f'This may be a private channel / group. Make {temp.B_NAME} an admin over there to index the files.')
    ...
    try:
        ...
        return await message.reply(f'Make Sure That {temp.B_NAME} An Admin In The Channel, if channel is private')
    if k.empty:
        return await message.reply(f'This may be group and {temp.B_NAME} not a admin of the group.')
    ...
    if type(chat_id) is int:
        try:
            ...
            return await message.reply(f'Make sure {temp.B_NAME} an admin in the chat and have permission to invite users.')
    else:
