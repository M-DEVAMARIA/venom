...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        if len(btn) > 10: 
            ...
            buttons = data['buttons'][0].copy()
            buttons.append(
            [InlineKeyboardButton(text="NEXT ⏩",callback_data=f"next_0_user_{keyword}"),InlineKeyboardButton(text=f"📃 Pages 1/{data['total']}",callback_data="pages")]
            )    
        else:
            buttons = btn
            buttons.append(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        ...
        typed = query.from_user.id or query.data.split("_")[3]
        pass 
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ident, index, user, keyword = query.data.split("_")
            try:
                data = BUTTONS[keyword]
            except KeyError:
