...
async def normalspellmode(message, template):
    ...
    buttons=[]
    buttons.append(let["configs"]["custom_button"])
    reply_button = eval(buttons) if not button==None else InlineKeyboardMarkup([[InlineKeyboardButton("🔍 GOOGLE ", url=f'https://www.google.com/search?q={search}'), InlineKeyboardButton("IMDB 🔎", url=f'https://www.imdb.com/search?q={search}')]])
