...
async def normalspellmode(message, template):
    ...
    reply_button = InlineKeyboardMarkup(list_to_str(buttons)) if not buttons==None else InlineKeyboardMarkup([[InlineKeyboardButton("🔍 GOOGLE ", url=f'https://www.google.com/search?q={search}'), InlineKeyboardButton("IMDB 🔎", url=f'https://www.imdb.com/search?q={search}')]])
