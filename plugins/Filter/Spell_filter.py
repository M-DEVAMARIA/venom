...
async def advancespellmode(msg):
    ...
    dump = DUPLICATES.get(query)
    if dump and dump.get("movielist"):
       movielist = dump.get("movielist")
       await msg.reply_text("using already stored")
    else:
