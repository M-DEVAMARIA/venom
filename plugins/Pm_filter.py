...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            ...
            await client.send_message(chat_id=message.from_user.id, text='sorry that movie not in my database try ')
            return
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close":
        ...
        await query.message.edit_text(Translation.ABOUT_TXT.format(time_formatter(time.time() - start_uptime), reply_markup=InlineKeyboardMarkup(
               [
                   [
